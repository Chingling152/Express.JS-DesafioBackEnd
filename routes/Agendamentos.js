            ...
            ser=>{
                if(!ser){
                    ...
                    if(h < hS || h > hF){
                        ...
                        Agendamento
                        .findAll({where:{servico,dataAgendada}})
                        .then(
                            age =>{
                                if(age){
                                    ...
                                    if(numeroVaga > 0 && numeroVaga <= ser.vagas){//não pode exceder a quantidade de vagas
                                        ...
                                        res.status(404).json({mensagemErro:`Não há vagas disponiveis para ${ser.titulo} no dia ${dataAgendada}`})
                                    }
                                }
                            }
                        )
                        .catch(k=>res.status(400).json({mensagemErro: k.errors[0].message || k}))
                    }
                }
            }
        ).catch(i=>{
