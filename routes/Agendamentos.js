    ...
    (req,res) =>{
        ...
        if(!token){
            ...
            jwt.verify(token, process.env.CHAVE_TOKEN,(err,decoded)=>{
                ...
                Agendamento
                    .findAll({include:[Servico],where:{cliente:parseInt(decoded.id)}})
                    .then(
                        age=>{
                            if(!age){
                                res.status(404).json({mensagemErro: `Você não agendou nenhum serviço`})
                            }else{
                                res.json(age)
                            }
                        }
...
router.post(
    ...
    (req,res)=>{
        ...
        if(!token){
            ...
            jwt.verify(token, process.env.CHAVE_TOKEN,(err,decoded)=>{
                if(err)return res.status(500).json({mensagemErro:"erro ao autenticar token"})
                var {dataAgendada,horario,servico} = req.body
                var cliente = decoded.id;
